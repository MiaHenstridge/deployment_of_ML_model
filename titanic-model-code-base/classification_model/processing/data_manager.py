...
from classification_model.config.core import DATASER_DIR, TRAINED_MODEL_DIR, config 
...
def get_first_cabin(row: Any) -> Union[str, float]:
	"""Get first value from cabin values"""
	try:
		return row.split()[0]
...
def get_title(passenger: str) -> str:
	"""Get title from passenger's name"""
	line = passenger
...
def pre_pipeline_preparation(*, dataframe: pd.DataFrame) -> pd.DataFrame:
	...
	# obtain title from passenger's name
...
def _load_raw_dataset(*, file_name:str) -> pd.DataFrame:
	dataframe = pd.read_csv(Path(f'{DATASER_DIR}/{file_name}'))
	return dataframe
